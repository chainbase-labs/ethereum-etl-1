...
class EthStreamerAdapter:
  ...
  def export_all(self, start_block, end_block):
    ...
    if self._should_export(EntityType.BLOCK) or self._should_export(
      ...
      blocks, transactions = self._export_blocks_and_transactions(start_block,
                                                                  end_block)
    ...
    enriched_transactions = (
      ...
      if EntityType.TRANSACTION in self.entity_types and self.chain[
        0] == "optimism"
      else enrich_transactions(transactions, receipts)
      if EntityType.TRANSACTION in self.entity_types
      else []
    )
    enriched_logs = enrich_logs(blocks,
                                logs) if EntityType.LOG in self.entity_types else []
    enriched_token_transfers = (
      enrich_token_transfers(blocks,
                             token_transfers) if EntityType.TOKEN_TRANSFER in self.entity_types else []
    )
    enriched_traces = (
    ...
    enriched_contracts = enrich_contracts(blocks,
                                          contracts) if EntityType.CONTRACT in self.entity_types else []
    enriched_tokens = enrich_tokens(blocks,
                                    tokens) if EntityType.TOKEN in self.entity_types else []

    logging.info("Exporting with " + type(self.item_exporter).__name__)

    reorg_stream_cdc_messages = self._get_reorg_cdc_streaming_message(
        start_block)
  ...
  def _get_reorg_cdc_streaming_message(self, start_block: int) -> list:
    ...
    if self.reorg_service.reorg_block is not None:
      ...
      if start_block <= reorg_block_number:
        reorg_block_range = self.reorg_service.reorg_block.get(
            "reorg_block_range")
        block_number_list = [int(item) for item in reorg_block_range.keys()]
        reorg_streaming_record = self.reorg_service.get_delete_record(
            block_number_list, self.entity_types)
  ...
  def _export_receipts_and_logs(self, transactions):
    ...
    job = ExportReceiptsJob(
        transaction_hashes_iterable=(transaction["hash"] for transaction in
                                     transactions),
        batch_size=self.batch_size,
        batch_web3_provider=self.batch_web3_provider,
        max_workers=self.max_workers,
  ...
  def _extract_token_transfers(self, logs):
    ...
    job = ExtractTokenTransfersJob(
        logs_iterable=logs, batch_size=self.batch_size,
        max_workers=self.max_workers, item_exporter=exporter
    )
    job.run()
  ...
  def _export_contracts(self, traces):
    ...
    job = ExtractContractsJob(
        traces_iterable=traces, batch_size=self.batch_size,
        max_workers=self.max_workers, item_exporter=exporter
    )
    job.run()
  ...
  def _should_export(self, entity_type):
    ...
    if entity_type == EntityType.TRANSACTION:
      return EntityType.TRANSACTION in self.entity_types or self._should_export(
          EntityType.LOG) or self._should_export(
          EntityType.TRACE) or self._should_export(EntityType.CONTRACT)
